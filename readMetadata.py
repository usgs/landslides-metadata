...
xl = pd.read_csv('/Users/kbiegel/Documents/EQIL/EQIL_Metadata.csv')
...
for i in range(2, a):
    ...
    metadata['metadata']['idinfo']['citation']['citeinfo']['geoform'] = 'Electronic'
    metadata['metadata']['idinfo']['citation']['citeinfo']['pubinfo']['pubplace'] = 'None'
    metadata['metadata']['idinfo']['citation']['citeinfo']['pubinfo']['publish'] = 'None'
    ...
    if pd.isnull(xl['source'].loc[i]):
        metadata['metadata']['distinfo']['distliab'] = 'These data and associated metadata were not acquired by the U.S. Geological Survey (USGS) and thus have not been reviewed for accuracy and completeness by the USGS.  They are presented here for convenience of the user only, as part of an effort to make published ground failure inventories more accessible from a single aggregated site. No warranty expressed or implied is made regarding the display or utility of the data on any other system or for general or scientific purposes, nor shall the act of distribution constitute any such warranty. The act of distribution shall not constitute any such warranty, and no responsibility is assumed by the USGS in the use of these data or related materials.'
    else:
    ...
    # Print xml to file
    dom = parseString(xml)
    filename = open('/Users/kbiegel/Documents/EQIL/xml/metadata_%s_%i.xml' % (xl['Inventory'].loc[i], i), 'w')
    ...
    # reopen file
    tree = ET.parse('/Users/kbiegel/Documents/EQIL/xml/metadata_%s_%i.xml' % (xl['Inventory'].loc[i], i))
    root = tree.getroot()
    ...
    tree.write('/Users/kbiegel/Documents/EQIL/xml/metadata_%s_%i.xml' % (xl['Inventory'].loc[i], i))

    # write changes to file
    dom1 = parse('/Users/kbiegel/Documents/EQIL/xml/metadata_%s_%i.xml' % (xl['Inventory'].loc[i], i))
    filename = open('/Users/kbiegel/Documents/EQIL/xml/metadata_%s_%i.xml' % (xl['Inventory'].loc[i], i), 'w')
    ...
    for q in root.iter('lworkcit'):
        ...
        for r in q.iter('publish'):
            tempstring += r.text + ', '
        tempstring += 'accessed May 15, 2017, '
        for r in q.iter('onlink'):
            tempstring += 'at ' + r.text
    tempstring += '.'
    string.append(str(tempstring))

filename = open('/Users/kbiegel/Documents/EQIL/citations.doc', 'w')
for i in range(2, a):
